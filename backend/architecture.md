...
| 7 行动 Action | 当日事件 + 市场研判 | 战略情报智囊团（§17）：情报适配为 intelligence_batch → 5 位专家并行分析 → 总参谋综合 → 决策报告（上中下三策）→ 派生行动 | `council_reports` + `action_items` |
...
| 12 条兵法策略库 | `knowledge/strategy_library.json` | 周大福海外市场专属计策（原 `library.py` 迁移而来） | 行业落地 playbook：把通用谋略落到珠宝出海的具体打法 |
...
| `items[].category` | 由 `event_type` 映射（competition→competition、pricing→price …） |
| `items[].event_summary` | `summary` |
| `items[].sentiment / impact_area` | 由 `impact_type` 推导 |
| `items[].confidence` | `confidence`（high / medium / low → 数值） |
| `items[].tags` | 由 `event_type` / 标题关键词派生 |
