...
| 文件 | 在本文中的作用 |
|---|---|
| [`CLAUDE.md`](CLAUDE.md) | data_probe 的定位、运行边界、probe 状态、踩坑经验和数据质量（主要参考） |
...
| [`output/CRAWL_SNAPSHOT_20260523.md`](output/CRAWL_SNAPSHOT_20260523.md) | 2026-05-23 实际跑出来的数据数量和市场覆盖 |
| [`output/CRAWL_SUMMARY_20260523.md`](output/CRAWL_SUMMARY_20260523.md) | 人工整理后的战略总结和质量判断 |
...
只要其中一环弱，最后的分析也会弱。例如 Google News 覆盖很广，但 RSS 条目只有标题和跳转链接，`raw_text` 为空；Federal Register 覆盖窄，但证据质量高；百度指数源头存在，但真实指数需要 JS/登录态。依据：`CLAUDE.md`、`sources.yaml`、`CRAWL_SNAPSHOT_20260523.md`。

---

## data_probe 现在到底爬了什么

截至 2026-05-23，JSONL 快照共 6 个 source、**1,483 条记录**（含 19 个 failed placeholders）。依据：`CRAWL_SNAPSHOT_20260523.md`。
...
**"有 source_id"不等于"高质量可分析"。** `google_news_rss` 有 654 条但分析价值停在标题级（GN 跳转链接，无正文）；`gdelt_doc` 约 50% 失败。意义是：源头已确认可访问，但质量限制已记录，下一步路径已有判断。依据：`CLAUDE.md`、`CRAWL_SNAPSHOT_20260523.md`。
...
失败占位不是垃圾。它告诉你某个源头"目前不能稳定生产化"的原因。百度指数需要 JS/登录态，Lazada 是 JS 渲染，Shopee 超时，Shopify atom feed 不是真的 feed，DDG 在 Cursor 沙盒 IP 下被封锁。依据：`CLAUDE.md`、`CRAWL_SNAPSHOT_20260523.md`。
...
这套架构的关键价值是"不依赖即时内部数据"。它不回答"昨天实际卖了多少"，而回答"外部环境发生了什么，哪些变化可能影响未来运营"。依据：`README.md`、`CLAUDE.md`、`CRAWL_SUMMARY_20260523.md`。
...
这三层不能混在一起。Reddit 上某个抱怨帖是事实，但它是否代表整体市场风险，需要结合声量、重复出现次数、平台、地区和业务重要性判断。依据：`utils.py`、`CRAWL_SUMMARY_20260523.md`。
