...
│   ├── probe_ecommerce.py / probe_tavily.py / probe_reddit.py
│   ├── probe_trends.py / probe_baidu_index.py
│   │                     # 全球 / 可选（输出 JSONL，PRD §2 18 字段 schema）
│   │
│   └── probe_global_news_enrich.py   # 独立 enricher：GN 标题 → 正文（见下）
└── output/
    ├── normalized/       # *.jsonl（全球）/ *.json（SG 旧版）/ *_enriched_*.jsonl
...
合计 **1,488 条记录**（含 22 个 failed placeholders），覆盖 9 市场。

| Probe | source_id | 状态 | 记录数 | 关键点 |
|---|---|---|---:|---|
| `probe_federal_register` | federal_register | ✅ 全通 | 135 | US 官方法规 API；质量最高 |
| `probe_global_news` | google_news_rss | ✅ 全通 | 654 | 9 市场 × 5 查询 × 15 条；URL 是 GN 跳转，正文需 enricher 补充 |
...
| `probe_baidu_index` | baidu_index | ⚠ 受限占位 | 1 | 页面可访问，但指数曲线依赖 JS + 登录态 |
| `probe_ecommerce` | ecommerce_announcements | ⚠ 大部分失败 | 4 | Shopify 不是真 atom；Amazon 反爬；Shopee 超时；Lazada JS |
...
### Google Trends / 百度指数（trends / baidu_index）

**Google Trends**（✅ 45 条）：pytrends，9 市场 × 5 关键词，7 天热度序列，`confidence=0.3`，仅方向性参考

**百度指数**（⚠ 受限）：页面可访问，但真实曲线依赖 JS + 登录态；当前只存 raw HTML 占位。生产化路径：官方授权 / 合规浏览器采集 / licensed data provider

### Google News 正文富化（probe_global_news_enrich.py）

`probe_global_news.py` 产出的 654 条记录 `url` 是 GN 跳转链接、`raw_text` 为空。
`probe_global_news_enrich.py` 尝试将其升级为正文级数据。

**已试过的 URL 解析路径**（2026-05-23）：

| 方法 | 结果 | 原因 |
|---|---|---|
| `urllib` 跟 GN 301 跳转 | ❌ | GN RSS URL 经 JS 二次跳转，`urlopen` 跟完仍在 `news.google.com` |
| base64 decode GN token | ❌ | `CBMi...` token 是不透明 protobuf blob，不含可解析 ASCII URL |
| DDG HTML（`html.duckduckgo.com`） | ⚠ 环境依赖 | Cursor 沙盒出口 IP 被 DDG 判定为 bot（返回 `anomaly.js`）；系统终端走 VPN 代理时**可用** |
| DDG Lite（`lite.duckduckgo.com`） | ❌ | 同样被 Cursor 沙盒 IP 拦截 |

**结论**：DDG 两步法是当前唯一可行路径，但必须从系统终端（iTerm2 / Terminal.app）运行——Cursor 沙盒用数据中心出口 IP（`:58423`），DDG 封锁；系统终端走本机 VPN（`:7897`），正常。

```bash
# 必须在系统终端里跑，不要通过 Cursor Shell/Run
cd /Users/leon/Developer/Aurum-Radar/data_probe && source .venv/bin/activate
python scripts/probe_global_news_enrich.py --max 20   # 先验证 20 条（约 2 分钟）
python scripts/probe_global_news_enrich.py             # 全量（约 30-40 分钟）
```

脚本启动时自动做 DDG 预检，检测到 bot 拦截直接退出并打印提示。
输出：`output/normalized/google_news_rss_enriched_YYYYMMDD.jsonl`，新增字段 `gn_url`（原链接）、`enrich_status`
